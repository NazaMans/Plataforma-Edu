...
export const crearCurso = async (nombre) => {
    const query = await pool.query("INSERT INTO cursos (nombre) VALUES ($1) RETURNING id, nombre, estado", [nombre]);
    return query.rows[0];
};
...
export const inscribirUsuarioACurso = async (id_curso, id_usuario) => {
    const query = await pool.query("INSERT INTO cursos_usuarios (id_curso, id_usuario) VALUES ($1, $2) RETURNING id_curso, id_usuario", [id_curso, id_usuario]);
    return query.rows[0];
};

//Editar curso
export const editarCurso = async (id_curso, nombre, estado) => {
    const query = await pool.query("UPDATE cursos SET nombre = $1, estado = $2 WHERE id = $3 RETURNING id, nombre, estado", [nombre, estado, id_curso]);
    return query.rows[0];
};

//Desinscribir usuario a curso
export const desinscribirUsuarioACurso = async (id_curso, id_usuario) => {
    const query = await pool.query("DELETE FROM cursos_usuarios WHERE id_curso = $1 AND id_usuario = $2 RETURNING id_curso, id_usuario", [id_curso, id_usuario]);
    return query.rows[0];
};
...
export const eliminarCurso = async (id_curso) => {
    const query = await pool.query("UPDATE cursos SET estado = false WHERE id = $1 RETURNING id, estado", [id_curso]);
    return query.rows[0];
};
